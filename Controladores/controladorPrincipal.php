            ...
            case 1:
                ...
                if (AccesoADatos::addPregunta($preguntados)) {
                    ...
                    if (AccesoADatos::addOpciones($opciones)) {
                        $_SESSION['mensaje'] = 'Has creado una pregunta.';
                        header('Location: ../Vistas/creaPreguntas.php');
                    }
                }
                break;
...
if (isset($_REQUEST['crearExamen'])) {
    ...
    if (isset($_SESSION['usuarioIniciado'])) {
        ...
        $examen = new Examen('id', $nombre, $usuarioAux->getId(), 'default', $fechaInicio, $fechaFin, $opcion);
...
/**
 * Carga la información de un examen y manda a la vista correspondiente
 */
if (isset($_REQUEST['asignarPreguntas'])) {
    $titulo = $_REQUEST['nombreExamen'];
    $id = $_REQUEST['idExamen'];
...
/**
 * Asigna una pregunta a un examen
 */
if (isset($_REQUEST['addPregunta'])) {
    ...
    if (AccesoADatos::addPreguntaExamen($idExamen, $idPregunta)) {
        $_SESSION['mensaje'] = 'Pregunta(s) añadida(s).';
        header('Location: ../Vistas/anadirPreguntas.php');
    }
}

/**
 * Carga la lista de examenes creados por un usuario y manda a la vista correspondiente
 */
...
/**
 * Manda borrar un examen determinado
 */
if (isset($_REQUEST['borrarExamen'])) {
    $idExamen = $_REQUEST['idExamen'];
    $mensaje = AccesoADatos::deleteExamen($idExamen);
    $_SESSION['mensaje'] = $mensaje;
    header('Location: ../Controladores/controladorPrincipal.php?irAExamenes=1');
}

/**
 * Carga los datos de un examen y manda a la vista correspondiente
 */
if (isset($_REQUEST['verExamen'])) {
    $idExamen = $_REQUEST['idExamen'];
    
    $examen = AccesoADatos::getExamen($idExamen);
    $preguntas = AccesoADatos::getPreguntasExamen($idExamen);
